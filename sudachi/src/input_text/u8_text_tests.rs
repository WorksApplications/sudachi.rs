...
fn build_mock_grammar() -> Grammar<'static> {
    ...
    let character_category = CharacterCategory::from_file(&PathBuf::from(CHAR_CATEGORY_FILE_PATH))
...
fn get_original_text() {
    let grammar = build_mock_grammar();
...
fn get_original_index() {
    ...
    for (i, &v) in [
        0, 2, 2, 5, 5, 5, 6, 7, 10, 10, 10, 11, 12, 13, 16, 16, 16, 19, 19, 19, 23, 23, 23, 23, 26,
        26, 26, 29, 29, 29, 32, 32, 32,
    ]
    .iter()
    .enumerate()
    {
...
fn replace_multiple_times() {
    ...
    for (i, &v) in [
        0, 2, 5, 6, 7, 10, 10, 10, 11, 12, 13, 16, 16, 16, 19, 19, 19, 23, 23, 23, 26, 26, 26, 32,
        32, 32,
    ]
    .iter()
    .enumerate()
